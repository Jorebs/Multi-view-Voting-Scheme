...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # Losses
            rpn_class_loss = KL.Lambda(lambda x: rpn_class_loss_graph(*x), name="rpn_class_losss")(
            ...
            outputs = [rpn_class_logits, rpn_class, rpn_bbox,
                       ...
                       mask_dice_loss,mask_scoring_loss]
    ...
    def compile(self, learning_rate, momentum):
        ...
        print('keras_model: ',self.keras_model.summary())
        ...
        for name in loss_names:
            ...
            loss = (
                tf.reduce_mean(layer.output, keepdims=True)
                * self.config.LOSS_WEIGHTS.get(name, 1.))
            #self.keras_model.add_metric(loss, name) #Christian: This line was 
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        #Christian: excluding double classification of the same instrument
        def intersection(i,j):
            if boxes[i,0] >= boxes[j,2] or boxes[j,0] >= boxes[i,2]:
                return False
            if boxes[i,1] >= boxes[j,3] or boxes[j,1] >= boxes[i,3]:
                return False
            in_top = min(boxes[i,2],boxes[j,2])
            in_bottom = max(boxes[i,0],boxes[j,0])
            in_right = min(boxes[i,3],boxes[j,3])
            in_left = max(boxes[i,1],boxes[j,1])
            
            in_area = (in_top - in_bottom)*(in_right-in_left)
            un_area = (boxes[i,2] - boxes[i,0]) * (boxes[i,3] - boxes[i,1]) + (boxes[j,2] - boxes[j,0]) * (boxes[j,3] - boxes[j,1]) - in_area
            if in_area/un_area>=0.5:
                return True
            else:
                return False
        ...
        while i < N:
            j=1
            while j < N:
                if  i != j and intersection(i,j):
                    if scores[i] >= scores[j]:
                        boxes = np.delete (boxes,j, axis=0)
                        class_ids = np.delete (class_ids,j,axis=0)
                    ...
                    if scores[i] < scores[j]:
                        ...
                        class_ids = np.delete(class_ids, i, axis=0)
        ...
        #Christian: excluding double predictions of the same class in the same image
...
def parse_image_meta(meta):
    ...
    scale = meta[:, 11]
    active_class_ids = meta[:, 12:] 																	### active_class_ids = meta[:, 12:] 
    return {
